...
@objc public class GrowthBookBuilder: NSObject, GrowthBookProtocol {
    ...
    @objc public func initializer() -> GrowthBookSDK {
        ...
        return GrowthBookSDK(context: gbContext, refreshHandler: refreshHandler, logLevel: growthBookBuilderModel.logLevel, networkDispatcher: networkDispatcher, cachingManager: cachingManager, ttlSeconds: ttlSeconds, fallbackFeatures: fallbackFeatures)
    }
}
...
@objc public class GrowthBookSDK: NSObject, FeaturesFlowDelegate {
    ...
    init(context: Context,
        ...
        self.ttlSeconds = ttlSeconds
        super.init()
        self.featureVM = FeaturesViewModel(delegate: self, dataSource: FeaturesDataSource(dispatcher: networkDispatcher), cachingManager: cachingManager, forceSynchronousSave: false, ttlSeconds: ttlSeconds, fallbackFeatures: fallbackFeatures)
